...
main(){
  try{
    ...
    // VMap2bdb test
    {
      ...
      assert_err(m.put(0,o1), "VMap2bdb::put: object does not exists: 0");
      assert_err(m.del(0), "VMap2bdb::del: object does not exists: 0");
      assert_err(m.get(0), "VMap2bdb::get: object does not exists: 0");

      assert_eq(m.get_types().size(), 0);

      assert_err(m.add(o1), "VMap2bdb::add: empty object");
      ...
      assert_err(m.put(1,o1), "VMap2bdb::put: object does not exists: 1");
      assert_err(m.del(1), "VMap2bdb::del: object does not exists: 1");
      assert_err(m.get(1), "VMap2bdb::get: object does not exists: 1");

      // update object
      o1.set_type(VMAP2_LINE, 0x2342);
      o1.dMultiLine::operator=(dMultiLine());
      assert_err(m.put(id, o1), "VMap2bdb::put: empty object");
